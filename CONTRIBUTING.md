...
3. **Make a plan.** Before writing code, write a short plan with your phases and keep it updated as you work. Keep planning and progress documents in their `.cio/` subfolder (`.cio/work/<feature>/` for chats, `.cio/specs/<feature-slug>/` in Engineering) and disposable scratch in `.cio/tmp/`; never pollute the repo root or the `.cio/` root.
...
- **Never commit ignored files** (`.cio/`, `out/`, `dist/`, `.env`, logs).
