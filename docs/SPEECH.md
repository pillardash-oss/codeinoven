...
Every Apple Silicon checkout resolves the three Swift speech workers (`mlx-worker`, `coreml-worker`, and `speech-capture-worker`) through `scripts/lib/speech-worker-build.ts` instead of compiling them from scratch. The packages live in `resources/speech/mlx-worker-src`, `resources/speech/coreml-worker-src`, and `resources/speech/native-capture-src`; their compiled output goes into `resources/speech/runtime/darwin-arm64/`, which is gitignored. Resolution is cheapest-first: an artifact whose state file at `.cio/tmp/speech-worker-state/<worker>.json` records the current source revision is reused, then a compiled artifact in the machine-local shared cache at `<config root>/build-cache/speech-workers/v1/<cache-name>/<key>/` is installed without compiling, and only a real miss compiles and publishes its result for every other checkout. The cache key is the SHA-256 content hash of `Package.swift`, `Package.resolved` when the package pins one, and every file under `Sources/`, combined with the platform and architecture, and the cache keeps only the two newest revisions per worker. The MLX Metal library (`mlx.metallib`) is cached separately, keyed by the pinned SHA-256 of the MLX Metal wheel it is extracted from, so replacing it never recompiles the worker. A cache hit means a new checkout does not compile at all. A real MLX compile has one prerequisite that Xcode does not install by default, the separable Xcode Metal toolchain, installed once with `xcodebuild -downloadComponent MetalToolchain`; the Core ML and native capture packages need no such component.
