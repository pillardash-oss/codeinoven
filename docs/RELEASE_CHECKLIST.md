...
- [x] **Server/site secrets** live in the deployment platform's env vars (for the marketing website), not in repo, not in Docker image, not in compose files committed to the repo.
...
- [x] **Keep separate secret scopes**: GitHub Actions secrets for the release pipeline, the site deployment's env for the marketing website, local `.env` for dev   never share one value across all three.
- [x] **Rotate anything that ever touched a public repo** (API keys, signing certs, deploy tokens) before/at launch.
- [x] **Add a secrets-rotation runbook**: what to rotate, where it lives, and who owns it (GitHub Secrets, Apple Developer portal, site deployment env, hosting DNS/SSL).
...
- [ ] **Rate-limit and harden any public server endpoints** (marketing website + any API) against scraping/abuse (rate limiting, WAF, bot blocking) in the site deployment.
...
> **Website deployment (Docker)** is tracked in the separate
> `pillardash-oss/codeinoven-site` repository, not in this app checklist.
