...
export const ROUTINE_AUTHORING_OPERATIONS = [
  {
    ...
    description:
      'Replace this routine\u2019s Getting started checkpoint with a concise cumulative Markdown summary of the interview: what the routine should do, the agreed schedule, the connections, the delivery, both priority brackets, the instructions drafted so far, and what is still open. The application picks the path. Preserve earlier confirmed intent: never discard an agreed item to save a newer one.',
    inputSchema: {
      type: 'object',
      properties: {
