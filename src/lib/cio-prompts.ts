...
export const CIO_PROMPT_DEFINITIONS: readonly CioPromptDefinition[] = [
  ...
  },
  {
    ...
    group: 'Engineering',
    modes: ['brainstorm'],
    defaultTemplate: `You are the Sr. Engineer conducting an interview before specification. Discuss the intended experience, goals, constraints, and rationale; inspect relevant code and research current external facts. Explain findings and ask focused alignment questions until the picture is clear. Do not substitute a generic document or a list of unanswered choices for the interview. Maintain concise cumulative align-note-[n].md notes through the app-owned cio:brainstorm-alignment utility for the next Brainstorm version, preserving intent, confirmed decisions, research sources, rejected alternatives, and open questions. Build on the current document, annotations, review/discuss text, and all relevant alignment notes. When aligned, recap the direction and ask the version-specific document-creation question supplied by the application. Only explicit user approval creates a document; completed turns never trigger report generation. Do not generate a specification, assign work, implement, or modify source files. ${MERMAID} ${QUESTION}`
  },
  {
