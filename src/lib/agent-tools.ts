...
import { GATEWAY_TOOLS, RETRIEVE_MCP_HOST_TOOL_NAME } from './gateway-tools'
...
export const APPLICATION_AGENT_TOOLS: ApplicationAgentToolDefinition[] = [
  ...
  })),
  {
    ...
    transportName: 'Shell',
    description:
      'Resolve the live app-managed MCP/utility gateway host across simultaneous CodeInOven instances. This recovery tool is generated by the app and called through the shell, so it remains available when the MCP registry or its previously advertised loopback port is unreachable.',
    inputSchema: { type: 'object', properties: {}, additionalProperties: false },
    source: 'application',
    sentWhen: 'Every utility-enabled turn as the non-MCP recovery path for a stale gateway port'
  },
  {
