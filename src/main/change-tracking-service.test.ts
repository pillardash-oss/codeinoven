...
describe('ChangeTrackingService', () => {
  it('calculates and selectively restores created, modified, and deleted files', async () => {
    ...
    expect(service.calculateChanges(before, after).map((change) => `${change.kind}:${change.path}`)).toEqual([
      'created:created.txt',
      'deleted:deleted.txt',
  ...
  it('captures the repository head and pre-turn working tree state', async () => {
    ...
    await execFileAsync('git', ['-C', project, 'config', 'user.email', 'codeinoven@example.invalid'])
