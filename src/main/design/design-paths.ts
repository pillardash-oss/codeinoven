...
/**
 * The one place a design capability turns a project-relative string into a
 * folder it may touch.
 *
 * Two operations resolve a folder from a model-written argument, `preview` to
 * serve it and `save-media` to write into it, so the default, the ceiling and
...
export const MAX_DESIGN_DIRECTORY_LENGTH = 1_024

export interface ResolvedDesignDirectory {
...
/**
 * Resolve one project-relative folder, defaulted to the design output root and
 * refused when it could name anything outside the project.
 */
export function resolveDesignDirectory(projectPath: string, raw: unknown): ResolvedDesignDirectory {
  let requested = DESIGN_OUTPUT_ROOT
  if (raw !== undefined) {
    if (typeof raw !== 'string') throw new Error('directory must be a project-relative path')
    const trimmed = raw.trim()
    if (trimmed.length > MAX_DESIGN_DIRECTORY_LENGTH) throw new Error('directory is too long')
