...
export class DeviceCredentialService {
   ...
   * marks the bootstrap used. Proof-of-possession over the provided transcript
   * is required unless the keys were generated by the desktop itself
  ...
  async enrollDevice(input: EnrollmentInput): Promise<EnrollOutcome> {
    ...
    const desktopGenerated = input.signingProof === '__desktop_generated__'
    if (!desktopGenerated) {
