...
export class UtilityOrchestrationService {
  ...
  async startTurn(request: UtilityTurnRequest): Promise<UtilityTurnGateway> {
    ...
    const recoveryInstruction = `The gateway host, port, and bearer token above are scoped to this turn only and rotate on every new turn. Never reuse a host, port, or token you remember from earlier in this conversation or a prior turn — always use the values given for the current turn. The always-active ${RETRIEVE_MCP_HOST_TOOL_NAME} utility is independent of MCP. If the advertised app gateway is unreachable, or you are starting a new turn without a freshly given gateway, call it through the shell with \`node ${shellQuote(retrieverPath)} ${shellQuote(request.sessionId)} ${shellQuote(id)}\`. It discovers the CodeInOven instance that owns this exact utility turn and returns the current \`mcpHost\`. Retry the original route against that host with the current turn's authorization header. Never print or persist the bearer token.`
