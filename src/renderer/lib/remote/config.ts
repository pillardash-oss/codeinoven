 ...
 * and delivered to the phone through the QR pairing URL (`?pair=<secret>`), so
...
export function buildRemoteConfig(
  ...
  // The relay only activates when fully provisioned. In production, a missing
  // URL/token disables the relay route (LAN-first still works) instead of
  // erroring — credentials are a deploy-time requirement, never invented.
