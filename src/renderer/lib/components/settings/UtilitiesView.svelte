<script lang="ts">
  ...
  // Loads the agent tool catalog once the Tools tab is first visited; the
  // store's own TTL cache makes every later visit (in this session) instant.
  $effect(() => {
    if (activeTab === 'tools') void agentToolsStore.load()
  })
</script>
...
{#snippet tagChip(tag: string)}
  ...
  {:else if tag === 'Global' || tag === 'Project'}
    <span>{scopeTagLabel(tag)}</span>
  {:else}
...
<div class="mx-auto max-w-5xl p-6 pb-24">
  ...
  {#if isListTab(activeTab)}
    <!-- Row 4 — search, entry count, refresh: one row, same position for every list tab. -->
