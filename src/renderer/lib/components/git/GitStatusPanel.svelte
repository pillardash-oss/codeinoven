  ...
  <div class="flex shrink-0 items-center gap-0.5 border-b border-border px-2 py-1">
    <button
      ...
      class={[
        ...
        activeTab === 'changes' ? 'bg-elevated text-foreground' : 'text-muted hover:text-foreground'
      ]}
    ...
    >
      ...
      {#if changes.length > 0}
        <span
          class="rounded bg-primary/15 px-1 py-0.5 text-[8px] font-semibold tabular-nums text-primary"
        >
          {changes.length}
        </span>
      {/if}
    ...
    <button
      ...
      class={[
        'flex items-center gap-1 rounded-md px-2 py-1 text-[10px] font-medium transition-colors',
        activeTab === 'history' ? 'bg-elevated text-foreground' : 'text-muted hover:text-foreground'
      ]}
      onclick={() => {
        activeTab = 'history'
        void loadHistory()
    ...
    >
      ...
      {#if gitState.branches.length > 0}
        <span class="text-[8px] tabular-nums text-dimmed">{gitState.branches.length}</span>
      {/if}
    </button>
  </div>
      ...
      {#if activeTab === 'changes'}
        ...
        {:else if status}
          ...
          {#if staged.length > 0}
            <div class="mb-2 overflow-hidden rounded-lg border border-border bg-surface">
              <div class="px-3 pt-2 pb-1">
                <textarea
                  class="min-h-14 w-full resize-none rounded-md border border-border bg-elevated px-2.5 py-2 font-mono text-[11px] leading-relaxed text-foreground outline-none placeholder:text-dimmed focus:border-primary"
          ...
          <div class="flex items-center gap-1.5">
            ...
            <button
              type="button"
              class="flex h-7 items-center gap-1 rounded-md border border-border px-2 text-[10px] font-medium text-muted transition-colors hover:bg-elevated hover:text-foreground disabled:opacity-40"
              ...
              <div class="border-t border-border px-3 py-2">
                ...
                <div class="mt-1.5 flex items-center justify-between gap-2">
                  <p class="text-[9px] leading-relaxed text-dimmed">
                    Merge/rebase can overwrite local changes.
                  </p>
                  <button
                    type="button"
                    class="shrink-0 rounded-md border border-border px-2 py-1 text-[10px] font-medium text-muted hover:bg-elevated hover:text-foreground disabled:opacity-40"
      ...
      {:else if activeTab === 'history'}
        ...
        {:else if commitHistory.length === 0}
          <div class="flex h-full flex-col items-center justify-center px-6 text-center">
            ...
            <p class="mt-1 text-[10px] text-dimmed">Make your first commit to see history here.</p>
          </div>
        {:else}
      ...
      {:else if activeTab === 'branches'}
        ...
        <div class="mt-2 space-y-0.5">
          {#each gitState.branches as branch (branch.name)}
            <button
              type="button"
              class="flex w-full items-center gap-2 rounded-lg px-2 py-1.5 text-left transition-colors hover:bg-elevated/50"
