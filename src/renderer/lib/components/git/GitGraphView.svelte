<script lang="ts">
  import { GitCommit as GitCommitIcon, Loader2, RefreshCw } from '@lucide/svelte'
  import { onMount } from 'svelte'
  import type { GitCommitInfo } from '$shared/types'
  import { relativeTime } from '$lib/format/relative-time'
  import {
    GRAPH_LANE_WIDTH,
    GRAPH_MAX_LANES,
    GRAPH_ROW_HEIGHT,
    assignGraphLanes,
    graphLaneColor,
    graphLaneX,
    graphRowEdges
  } from '$lib/git-graph'
  import { gitState } from '$lib/stores/git.svelte'

  interface Props {
    projectId: string
    /** Local commits not yet on the upstream, marked on the graph. */
    unpushedCount: number
    /** Upstream ref name, for the pushed-boundary label. */
    upstream: string | null
    onSelectCommit: (commit: GitCommitInfo) => void
  }

  let { projectId, unpushedCount, upstream, onSelectCommit }: Props = $props()

  /**
   * Bounded so the work stays cheap. This component is mounted only while the
   * Graph tab is open, so the lane walk never runs for users who never look.
   */
  const GRAPH_PAGE_SIZE = 150

  let commits = $state<GitCommitInfo[]>([])
  let loading = $state(true)
  let loadingMore = $state(false)
  let hasMore = $state(true)
  /** Invalidates in-flight pages so a stale scope's commits never land here. */
  let requestId = 0

  // The single forward pass is O(commits) and re-runs only when the list
  // changes; appending an older page leaves the rows above untouched.
  const rows = $derived.by(() => assignGraphLanes(commits))

  const laneColumnWidth = $derived.by(() => {
    let widest = 1
    for (const row of rows) widest = Math.max(widest, row.laneCount)
    return Math.min(widest, GRAPH_MAX_LANES) * GRAPH_LANE_WIDTH
  })

  /** True once more lanes are in use than the column draws. */
  const lanesClipped = $derived(rows.some((row) => row.laneCount > GRAPH_MAX_LANES))

  async function loadFirstPage(): Promise<void> {
    loading = true
    requestId += 1
    const request = requestId
    try {
      const page = await gitState.getLog(projectId, GRAPH_PAGE_SIZE, 0)
      if (request !== requestId) return
      commits = page
      hasMore = page.length === GRAPH_PAGE_SIZE
    } finally {
      if (request === requestId) loading = false
    }
  }

  async function loadOlderCommits(): Promise<void> {
    if (loading || loadingMore || !hasMore) return
    loadingMore = true
    requestId += 1
    const request = requestId
    try {
      const page = await gitState.getLog(projectId, GRAPH_PAGE_SIZE, commits.length)
      if (request !== requestId) return
      commits = [...commits, ...page]
      hasMore = page.length === GRAPH_PAGE_SIZE
    } finally {
      if (request === requestId) loadingMore = false
    }
  }

  onMount(() => {
    void loadFirstPage()
  })
</script>

<div class="flex flex-col" data-region="git-graph">
  <div class="sticky top-0 z-10 flex items-center gap-2 border-b border-border bg-app px-3 py-2">
    <GitCommitIcon size={11} class="shrink-0 text-dimmed" aria-hidden="true" />
    <span class="text-[0.5625rem] font-semibold uppercase tracking-wide text-muted">
      Commit graph
    </span>
    <span class="text-[0.5rem] tabular-nums text-dimmed">{commits.length}</span>
    {#if lanesClipped}
      <span
        class="text-[0.5rem] text-dimmed"
        title={`Only the first ${GRAPH_MAX_LANES} lanes are drawn`}
      >
        first {GRAPH_MAX_LANES} lanes
      </span>
    {/if}
    <span class="flex-1"></span>
    <button
      type="button"
      class="flex h-6 w-6 shrink-0 items-center justify-center rounded text-dimmed transition-colors hover:bg-elevated hover:text-foreground disabled:opacity-40"
      title="Reload the commit graph"
      aria-label="Reload the commit graph"
      disabled={loading}
      onclick={() => void loadFirstPage()}
    >
      <RefreshCw size={12} class={loading ? 'animate-spin' : ''} />
    </button>
  </div>

  {#if loading}
    <div class="flex items-center justify-center gap-2 py-10 text-xs text-dimmed">
      <Loader2 size={14} class="animate-spin" />
      Drawing the graph
    </div>
  {:else if rows.length === 0}
    <div class="flex flex-col items-center justify-center py-12 text-center">
      <GitCommitIcon size={22} class="mx-auto mb-2 text-dimmed" />
      <p class="text-xs font-medium text-muted">No commits to draw</p>
      <p class="mt-1 text-[0.625rem] text-dimmed">This branch has no history yet.</p>
    </div>
  {:else}
    {#each rows as row, index (row.commit.hash)}
      {@const edges = graphRowEdges(row)}
      {#if unpushedCount > 0 && index === unpushedCount}
        <div class="flex items-center gap-2 px-2 py-1">
          <span class="h-px flex-1 bg-border"></span>
          <span class="shrink-0 text-[0.5625rem] font-medium text-dimmed">
            Pushed to {upstream ?? 'the remote'}
          </span>
          <span class="h-px flex-1 bg-border"></span>
        </div>
      {/if}
      <div class="flex items-stretch hover:bg-elevated/50">
        <svg
          width={laneColumnWidth}
          height={GRAPH_ROW_HEIGHT}
          class="shrink-0 overflow-hidden"
          aria-hidden="true"
        >
          {#each edges as edge, edgeIndex (edgeIndex)}
            <path d={edge.path} fill="none" stroke={edge.color} stroke-width="1.5" />
          {/each}
          <circle
            cx={graphLaneX(row.lane)}
            cy={GRAPH_ROW_HEIGHT / 2}
            r="3"
            fill={graphLaneColor(row.lane)}
            stroke="var(--color-app)"
            stroke-width="1.5"
          />
        </svg>
        <button
          type="button"
          class={[
            'flex min-w-0 flex-1 items-center gap-2 px-2 text-left transition-colors',
            index < unpushedCount ? 'text-muted' : 'text-foreground'
          ]}
          style={`height: ${GRAPH_ROW_HEIGHT}px`}
          title={`View ${row.commit.shortHash}: ${row.commit.message.split('\n')[0] ?? ''}`}
          aria-label={`View commit ${row.commit.shortHash}: ${row.commit.message.split('\n')[0] ?? ''}`}
          onclick={() => onSelectCommit(row.commit)}
        >
          <span class="min-w-0 flex-1 truncate text-[0.6875rem] leading-snug">
            {row.commit.message.split('\n')[0]}
          </span>
          {#each row.commit.refs as ref (ref.head ? `head:${ref.name}` : `${ref.kind}:${ref.name}`)}
            <span
              class={[
                'shrink-0 rounded-sm px-1 py-px text-[0.5rem] font-medium',
                ref.head
                  ? 'bg-primary text-on-primary'
                  : ref.kind === 'tag'
                    ? 'bg-accent/15 text-accent'
                    : 'bg-primary/15 text-primary'
              ]}
            >
              {ref.name}
            </span>
          {/each}
          <span class="shrink-0 font-mono text-[0.5625rem] text-dimmed">
            {row.commit.shortHash}
          </span>
          <span class="shrink-0 text-[0.5625rem] text-dimmed">
            {relativeTime(row.commit.date)}
          </span>
        </button>
      </div>
    {/each}

    <div class="px-2 py-3">
      {#if loadingMore}
        <div class="flex items-center justify-center gap-2 text-[0.625rem] text-dimmed">
          <Loader2 size={12} class="animate-spin" />
          Loading older commits
        </div>
      {:else if hasMore}
        <button
          type="button"
          class="flex h-7 w-full items-center justify-center rounded-sm border border-border text-[0.625rem] font-medium text-muted transition-colors hover:bg-elevated hover:text-foreground"
          onclick={() => void loadOlderCommits()}
        >
          Load older commits
        </button>
      {:else}
        <p class="text-center text-[0.5625rem] text-dimmed">Start of history</p>
      {/if}
    </div>
  {/if}
</div>
