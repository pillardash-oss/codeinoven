<script lang="ts">
  ...
  let { hunk, anchor = null, rows = HUNK_CONTEXT_ROWS, class: className = '' }: Props = $props()
   ...
   */
  const clamped = $derived(hunkWindow(hunk, anchor, rows))
  const window = $derived(expanded ? hunkWindow(hunk, anchor, null) : clamped)
   ...
   */
  const numbered = $derived(window.lines.some((line: HunkLine) => line.number !== null))
