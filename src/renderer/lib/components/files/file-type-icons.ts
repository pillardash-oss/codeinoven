import { CIO_ICON_DATA_URI, getCioIconMarkup, isCioFolderName } from './cio-folder-icons'
 ...
 *
 * The `.cio` scratch folder carries the CodeInOven app icon instead of a folder
 * glyph: a collapsed and an expanded `.cio` folder are the same folder either
 * way, so `open` does not change the icon.
 */
export async function getFolderTypeIconDataUri(name: string, open = false): Promise<string | null> {
  if (isCioFolderName(name)) return CIO_ICON_DATA_URI
 ...
 * decodes back to markup. The CodeInOven app icon is not one of them: it carries
 * live gradient ids, so it is built by `getCioIconMarkup` instead of being
 * decoded from a shared `data:` URI whose ids would collide across instances. */
export async function getInlineFolderTypeIconSvg(name: string): Promise<string> {
  if (isCioFolderName(name)) return getCioIconMarkup()
